...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        if (SYSTEMVERSION < 8.0) {
            ...
            CGSize lableSize = [self.labelText boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:attributes context:nil].size;
            
            lHeight = lableSize.height;
            jsWidth = lableSize.width;
            
        }
        
        float lWidth ;
        
        if (jsWidth <= (frame.size.width - 4 * margin)) {
            lWidth = jsWidth;
        }
        else {
...
#warning MBProgessHUD
        ...
        if (nil != self.detailsLabelText) {
            ...
            if (SYSTEMVERSION < 8.0) {
                ...
                labelSize = [self.labelText boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:attributes context:nil].size;
