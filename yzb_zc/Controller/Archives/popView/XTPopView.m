...
{
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(-5, 0, self.backGoundView.frame.size.width - 5, self.backGoundView.frame.size.height) style:UITableViewStylePlain];
        _tableView.dataSource = self;
        _tableView.backgroundColor = [UIColor clearColor];
        _tableView.separatorColor = [UIColor groupTableViewBackgroundColor];///分割线颜色
        _tableView.separatorInset = UIEdgeInsetsMake(0,0, 0, 0);///设置分割线的左右距离
